...
var chars = require('./chars')
  ...
  , rules = {
      ...
      , 'headline'       : { fn : headline, re : /^([-A-Z0-9 .][-A-Z0-9 .()]*\s+)\*/ }
      , 'header'         : { fn : header, re : /^((.*)~)[\r\n]?$/ }
      , 'tag-name'       : { fn : tagName, re : /(\*([#-)!+-~]+)\*)(\s|$)/ }
      , 'tag-link'       : { fn : tagLink, re : /\|([#-)!+-~]+)\|/ }
      , 'vim-option'     : { fn : vimOption, re : /(('[a-z][a-z]+')|('t_..'))/ }
      , 'vim-version'    : { fn : vimVersion, re : /Vim version [0-9][0-9.a-z]*/ }
      ...
      , 'code-start'     : { fn : codeStart, re : /((^|\s)(>))[\r\n]?$/ }
      , 'code-close'     : { fn : codeClose, re : /^(<)[\r\n]?$/ }
...
function todoLine (match, input) {
    return ruleResult(match, input, 'todo-line', 1, 1);
}

function errorLine (match, input) {
    return ruleResult(match, input, 'error-line', 1, 1);
}
...
function codeClose (match, input) {
    return ruleResult(match, input, 'code-close', -1, 1);
}
...
function vimVersion (match, input) {
    return ruleResult(match, input, 'vim-version');
}
